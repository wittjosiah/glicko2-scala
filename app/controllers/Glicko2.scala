...
class Glicko2(tau: Float = 0.75) {
  ...
  def outcomeRating(player: Player, matches: List[Match]): Double = {
    ...
    g(opponent.ratingDeviation) * (score - E(player.rating, opponent.rating, opponent.ratingDeviation)) +
      outcomeRating(player, matches.tail)
  }
